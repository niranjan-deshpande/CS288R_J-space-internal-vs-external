...
python3 scripts/run_faithfulness.py --source jspace-med-B16384 --mode jspace --band $MED_BAND --k $MED_K --n 50 --batch 4

date; echo "=== (F2) adaptive budget points (bins 0-2, med + random-s0) ==="
for BIN in 0 1 2; do
  for BUD in 1024 4096; do
    $REUSE --source-tag jspace-med-B16384 --tag jspace-med-B$BUD-bin$BIN --budget $BUD --mode jspace --band $MED_BAND --k $MED_K --datasets gsm8k,math,aime --problems results/bin$BIN.json --batch 16 --kv-budget 55 --staged-source
    $REUSE --source-tag random-s0-B16384 --tag random-s0-B$BUD-bin$BIN --budget $BUD --mode random --band $MED_BAND --k $MED_K --rand-seed 0 --datasets gsm8k,math,aime --problems results/bin$BIN.json --batch 16 --kv-budget 55 --staged-source
  done
done
